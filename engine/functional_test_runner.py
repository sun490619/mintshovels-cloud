...
def safe_json_load(filepath, label="数据文件"):
    """安全加载 JSON 文件，带完整异常保护"""
    try:
        with open(filepath, "r", encoding="utf-8") as f:
            return json.load(f)
    except FileNotFoundError:
        print(f"⚠️ {label} 不存在: {filepath}")
        return None
    except json.JSONDecodeError as e:
        print(f"⚠️ {label} JSON 解析失败: {e}")
        return None
    except PermissionError:
        print(f"⚠️ {label} 无读取权限: {filepath}")
        return None
    except Exception as e:
        print(f"⚠️ {label} 加载异常: {e}")
        return None
...
def full_scan(tools_path: str = TOOLS_PATH, online_check: bool = False, sample: int = 0) -> dict:
    ...
    tools = safe_json_load(tools_path, "工具数据库")
    if not tools:
        return {"ok": False, "total": 0, "passed": 0, "failed": 0, "results": [], "error": "无法加载工具数据"}
    total = len(tools)
    ...
    if total == 0 or is_audit_only:
        ...
        if archive_path:
            tools = safe_json_load(archive_path, "归档工具-备用数据源") or []
            if tools:
                total = len(tools)
...
def health_check_snapshot() -> dict:
    ...
    tools = safe_json_load(tools_path, "工具数据库-门禁")
    if not tools:
        return {"ok": False, "detail": "无法加载工具数据", "hollow_count": 0, "total": 0, "health_rate": 0}
    total = len(tools)
    ...
    if total == 0:
        archive_path = _find_latest_archive()
        if archive_path:
            tools = safe_json_load(archive_path, "归档工具-门禁备用") or []
            if tools:
                total = len(tools)
            file_source = os.path.basename(archive_path)[:60]
        else:
    ...
    if actual_files == 0 and hollow_count > 0:
        ...
        if archive_path and "1558" in os.path.basename(archive_path):
            try:
                archive_tools = safe_json_load(archive_path, "1558归档")
                if archive_tools is None:
                    raise ValueError("归档加载失败")
                arch_hollow = 0
...
def main():
    ...
    # ── 单工具测试模式 ──
    if args.tool_id:
        tools = safe_json_load(TOOLS_PATH, "工具库") or []
        target = None
        for t in tools:
