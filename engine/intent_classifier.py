...
class IntentClassifier:
    ...
    def score_data_closure(self, text: str) -> int:
        ...
        # 额外加分：包含技术标准名称 + 动词
        if re.search(r'(JSON|CSV|PDF|XML|HTML|CSS|SQL|URL|PNG|JPG|JPEG|GIF|WebP|SVG|HEIC).*(格式|化|处理|验证|解析|转|convert)', text, re.I):
            score += 10
        ...
        if re.search(r'(计算|换算|转换|转化).*(利率|汇率|温度|长度|重量|面积|体积|BMI|贷款|月供|利息)', text):
            score += 10
        # 图片格式转换专项
        if re.search(fr'({_wb}(?:WebP|PNG|JPG|JPEG|GIF|SVG|HEIC|BMP|TIFF|AVIF){_we}).*(转|转成|转为|转换|convert|to).*({_wb}(?:WebP|PNG|JPG|JPEG|GIF|SVG|HEIC|BMP|TIFF|AVIF){_we})', text, re.I):
            score += 10
        # 短名称但包含工具名词的加分
        ...
        if re.search(r'\b(Image|Photo|Video|Audio|File|PDF|URL|Link|Code|Text|Price|Data|DNS|IP|Crypto|Stock)\b\s+\b(Upscaler|Tracker|Shortener|Converter|Generator|Editor|Analyzer|Checker|Manager|Monitor|Downloader|Extractor|Creator|Builder|Viewer|Finder|Scanner)\b', text, re.I):
            score += 15
        # 🆕 GitHub式命名: 全小写+连字符的技术命名约定 → 大概率是工具
        if re.match(r'^[a-z][a-z0-9_.-]{3,}[a-z0-9]$', text_lower) and '-' in text:
            score += 12
        # 🆕 明确提到 tool/tools/工具 的文本 → 必然与工具有关
        if re.search(r'\b(tool|tools|utility|utilities|plugin|extension|library|package|app)\b', text_lower):
            score += 12
    ...
    def score_computation_path(self, text: str) -> int:
        ...
        # 🆕 GitHub式连字符命名 (如 gemini-code-assist, daily-stock-analysis)
        text_lower = text.lower()
        if re.match(r'^[a-z][a-z0-9_.-]{3,}[a-z0-9]$', text_lower) and '-' in text:
            score += 8
        # 🆕 明确提及 tool/tools/工具/plugin/extension 的文本
        if re.search(r'\b(tool|tools|toolkit|plugin|extension|library|package|app|application)\b', text_lower):
            score += 8
        ...
        computable = [
            ...
            (r'(Emoji|表情).*(生成|随机)', 5),
            # 图片格式转换
            (r'(转|转换|转为|convert).*(WebP|PNG|JPG|JPEG|GIF|SVG|HEIC|BMP|TIFF|AVIF|格式)', 10),
            (r'.*(WebP|PNG|JPG|JPEG|GIF|SVG|HEIC|BMP|TIFF|AVIF).*(转|转成|转为|转换|convert|to).*(WebP|PNG|JPG|JPEG|GIF|SVG|HEIC|BMP|TIFF|AVIF)', 10),
        ]
    ...
    def _generate_canonical_name(self, text: str, intent: str) -> str:
        ...
        if intent == "calculator":
            # 图片格式转换检测 (WebP→PNG, HEIC→JPG 等)
            img_formats = r'(?:WebP|PNG|JPG|JPEG|GIF|SVG|HEIC|BMP|TIFF|AVIF|图片|照片)'
            img_match = re.search(fr'({img_formats})\s*(?:转|转成|转为|转换|convert|to)\s*({img_formats})', text, re.I)
            if img_match:
                f1, f2 = img_match.group(1), img_match.group(2)
                if f1 in ('图片', '照片'):
                    f1 = '图片'
                elif f1 in ('JPG', 'JPEG'):
                    f1 = 'JPG'
            ...
            match = re.search(r'[\u4e00-\u9fff]{2,6}(?:计算|换算|转换|转)', words)
            if match:
                prefix = match.group()
                if not prefix.endswith(('器', '工具')):
                    prefix = prefix.replace('转', '转换')
                return f"{prefix}器"
            return f"多功能计算器"
